...
## Builder stage: install Python deps and system libraries once.
...
# Install build and runtime libraries required by OCR/NLP toolchain.
RUN apt-get update \
...
# Collect native runtime dependencies for the distroless image.
